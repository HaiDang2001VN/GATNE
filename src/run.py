...
def train_model(network_data, feature_dic, log_name):
    ...
    with graph.as_default():
        ...
        if feature_dic is not None:
            ...
            linear = tf.compat.v1.layers.Dense(units=embedding_size, activation=tf.nn.tanh, use_bias=True)

            embed_trans = tf.Variable(tf.random.truncated_normal([feature_dim, embedding_size], stddev=1.0 / math.sqrt(embedding_size)))
        ...
        train_types = tf.compat.v1.placeholder(tf.int32, shape=[None])
        node_neigh = tf.compat.v1.placeholder(tf.int32, shape=[None, edge_type_count, neighbor_samples])
        
        # Look up embeddings for nodes
        if feature_dic is not None:
            node_embed = tf.nn.embedding_lookup(params=node_features, ids=train_inputs)
            node_embed = tf.matmul(node_embed, embed_trans)
        else:
            node_embed = tf.nn.embedding_lookup(params=node_embeddings, ids=train_inputs)
        
        if feature_dic is not None:
            node_embed_neighbors = tf.nn.embedding_lookup(params=node_features, ids=node_neigh)
            ...
            node_type_embed = tf.transpose(a=tf.reduce_mean(input_tensor=tf.reshape(node_embed_tmp, [edge_type_count, -1, neighbor_samples, embedding_u_size]), axis=2), perm=[1,0,2])
        else:
            node_embed_neighbors = tf.nn.embedding_lookup(params=node_type_embeddings, ids=node_neigh)
            ...
            node_type_embed = tf.transpose(a=tf.reduce_mean(input_tensor=node_embed_tmp, axis=2), perm=[1,0,2])

        trans_w = tf.nn.embedding_lookup(params=trans_weights, ids=train_types)
        trans_w_s1 = tf.nn.embedding_lookup(params=trans_weights_s1, ids=train_types)
        trans_w_s2 = tf.nn.embedding_lookup(params=trans_weights_s2, ids=train_types)
        ...
        if feature_dic is not None:
            node_feat = tf.nn.embedding_lookup(params=node_features, ids=train_inputs)
        ...
        plot_loss = tf.compat.v1.summary.scalar("loss", loss)

        # Optimizer.
        optimizer = tf.compat.v1.train.AdamOptimizer().minimize(loss, global_step=global_step)

        # Add ops to save and restore all the variables.
        # saver = tf.train.Saver(max_to_keep=20)

        merged = tf.compat.v1.summary.merge_all(key=tf.compat.v1.GraphKeys.SUMMARIES)
    ...
    with tf.compat.v1.Session(graph=graph) as sess:
        writer = tf.compat.v1.summary.FileWriter("./runs/" + log_name, sess.graph) # tensorboard --logdir=./runs
        sess.run(init)
